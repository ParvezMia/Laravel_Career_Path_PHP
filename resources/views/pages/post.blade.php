            ...
            <article class="bg-white border-2 border-black rounded-lg shadow mx-auto max-w-none px-4 py-5 sm:px-6">
                ...
                <header>
                    <div class="flex items-center justify-between">
                        ...
                        @if ($post->user_post_user_uuid == auth()->user()->uuid)
                            ...
                            <div class="flex flex-shrink-0 self-center" x-data="{ open: false }">
                                <div class="relative inline-block text-left">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        tabindex="-1">
                                        <a href="{{ route('home.post.edit', ['id' => $post->uuid]) }}"
                                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem"
                                            tabindex="-1" id="user-menu-item-0">Edit</a>
                                        <form action="{{ route('home.post.delete', ['id' => $post->uuid]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                ...
                <div class="py-4 text-gray-700 font-normal">
                    <p>
                        {!! nl2br($post->user_post_description) !!}
                    </p>
                </div>
                @php
                    $time = new Carbon\Carbon($post->created_at);
                    $created_at = '';
                    if ($time->isPast()) {
                        if ($time->diffInMinutes() < 60) {
                            $created_at = $time->diffInMinutes() . ' Minutes Ago';
                        } elseif ($time->diffInHours() < 24) {
                            $created_at = $time->diffInHours() . ' Hours Ago';
                        } else {
